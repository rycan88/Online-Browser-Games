    ...
    return (
            ...
            { remainingCardCount > 0 && 
                [...Array(Number(remainingCardCount))].map((_sym, index) => {
                    return (
                        <div className={`absolute ${(index === remainingCardCount - 1) ? "rounded-lg border-[0.5px] border-black" : "rounded-lg border-l-[0.5px] border-b-[0.5px]"} ${(index === remainingCardCount - 1 && canPick) && "hover:translate-x-[4px] hover:-translate-y-[4px]"}`}
                            style={{
