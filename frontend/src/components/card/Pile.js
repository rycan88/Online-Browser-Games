...
export const Pile = ({canPick, clickEvent, pile, pileElement, name, cardOutline=<CardOutline width={150}/>}) => {
    return (
        <div className={`${name} relative ${canPick && "hover:cursor-pointer"}`}
            ...
            { pile.map((card, index) => {
                    return (
                        <div className={`absolute rounded-[5%] ${(index === pile.length - 1) ? "border-[0.5px] border-black" : " border-l-[0.5px] border-b-[0.5px]"} ${(index === pile.length - 1 && canPick) && "hover:translate-x-[4px] hover:-translate-y-[4px]"}`}
                            style={{
